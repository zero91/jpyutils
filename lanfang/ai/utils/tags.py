...
def is_valid_iob(tags, version='IOB2'):
  ...
  for i, tag in enumerate(tags):
    ...
    pos, tag_name = fields
    if i == 0 or tags[i - 1].split('-') not in [['I', tag_name],
                                                ['B', tag_name]]:
...
def is_valid_iobes(tags):
  ...
  for i, tag in enumerate(tags):
    ...
    if pos in ['I', 'E'] and (i == 0 or tags[i - 1].split('-') not in [
                                ['B', tag_name], ['I', tag_name]]):
      return False
  return True
...
def iob1_to_iob2(tags):
  ...
  for i, tag in enumerate(new_tags):
    ...
    if i == 0 or new_tags[i - 1] == 'O' \
              or new_tags[i - 1].split('-')[1] != tag_name:
...
def is_start_chunk(prev_tag, cur_tag):
  ...
  if cur_tag_pos in ['B', 'S'] \
          or cur_tag_name != prev_tag_name \
          or prev_tag_pos in ['E', 'S']:
    return True
  return False
...
def is_end_chunk(cur_tag, next_tag):
  ...
  if cur_tag_pos in ['E', 'S'] \
          or next_tag_name != cur_tag_name \
          or next_tag_pos in ['B', 'S']:
    return True
  return False

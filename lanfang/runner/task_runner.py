...
class TaskRunner(threading.Thread):
    ...
    target: list, str
        ...
        By default, the program to be executed is the first item if 'target' is a sequence.

        The shell argument (which defaults to False) specifies whether to use the shell as the
        program to execute. If shell is True, it is recommended to pass 'target' as a string
    ...
    encoding: str
        ...
        The stdout outputs of the command will be treated as the command return value,
        and is parsed as an json dump string using ecoding as 'encoding' specifies.
    ...
    def run(self):
        ...
        if self._m_popen_kwargs.get("stdout") in [subprocess.PIPE, subprocess.DEVNULL]:
            stdout_stream = None
        else:
        ...
        while not self._m_stop_flag.is_set() and self._m_try_num < self._m_retry_limit:
            ...
            self._m_run_process = subprocess.Popen(self._target, **self._m_popen_kwargs)
            ...
            A dictionnary which contains information of task, including:
                {
                    ...
                    return: return value of the task which is extracted from stdout output,

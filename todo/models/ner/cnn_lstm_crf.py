...
class CNNBiLSTMCRFModel(abc.ABC):
    ...
    def _build_cnn_features(self):
        self._ph_alpha_ids = tf.placeholder(
            tf.int32,
            [None, self._m_config["max_sequence_length"], self._m_config["max_word_length"]],
            name="ph_alpha_ids"
        )
        self._m_alpha_ids_flat = tf.reshape(
            self._ph_alpha_ids,
            [-1, self._m_config["max_sequence_length"] * self._m_config["max_word_length"]],
            name="alpha_ids_flat"
        )

        with tf.device('/cpu:0'), tf.name_scope("alpha_embedding"):
            self._m_alpha_embeddings = tf.Variable(
                tf.random_uniform(
                    [self._m_config["alpha_size"], self._m_config["alpha_dim"]], -1, 1),
                trainable=True,
                name="alpha_embedding"
            )
            self._m_alpha_features = tf.nn.embedding_lookup(
                self._m_alpha_embeddings,
                self._m_alpha_ids_flat,
                name="alpha_features"
            )
            # [batch_size, max_sequence_length * max_word_length, alpha_dim]
            self._m_alpha_features_dropout = tf.nn.dropout(
                self._m_alpha_features,
                self._m_config["keep_prob"],
                name="alpha_features_dropout"
            )

        with tf.name_scope("alpha_conv_maxpool"):
            filter_shape = [
                self._m_config["filter_size"],
                self._m_config["alpha_dim"],
                self._m_config["num_filters"]
            ]
            W_conv = tf.Variable(tf.truncated_normal(filter_shape, stddev=0.1), name="W_conv")
            b_conv = tf.Variable(
                tf.constant(0.1, shape=[self._m_config["num_filters"]]),
                name="b_conv"
            )

            # [batch_size, max_sequence_length * max_word_length, num_filters]
            conv = tf.nn.conv1d(
                self._m_alpha_features_dropout,
                W_conv,
                stride=1,
                padding="SAME",
                name="conv"
            )
            # [batch_size, max_sequence_length * max_word_length, num_filters, 1]
            h_expand = tf.expand_dims(conv, -1)
            #ksize=[1, self._m_config["max_word_length"], self._m_config["max_word_length"], 1, 1]
            ksize=[1, self._m_config["max_word_length"], 1, 1]
            pooled = tf.nn.max_pool(
                h_expand,
                ksize=ksize,
                strides=[1, self._m_config["max_word_length"], 1, 1],
                padding='SAME',
                name="pooled"
            )
            self._m_alpha_cnn_features = tf.reshape(
                pooled,
                [-1, self._m_config["max_sequence_length"], self._m_config["num_filters"]],
                name="char_pool_flat"
            )
